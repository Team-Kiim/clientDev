...
export default function CategorySelector({ selectedCategories, updateSelectedCategories }: Props) {
    ...
    return (
        <div className={'flex w-full flex-col gap-y-2'}>
            <h3 className={'mx-1 text-[0.95rem] font-bold'}>카테고리 선택</h3>
                    ...
                    <ul className={'flex w-full items-center gap-x-2 overflow-x-scroll scrollbar-hide'}>
                        {CATEGORIES.map(category => {
                            return (
                                <li
                                    key={category.value}
                                    className={`shrink-0 cursor-pointer rounded-lg px-3 py-2 text-[0.9rem] ${category.value === selectedParentCategory ? 'bg-gray-100 font-bold' : 'bg-white font-normal'}`}
                                    onClick={() => {
