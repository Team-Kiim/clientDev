    ...
    skillCategoryList: [
        {
            ...
            childSkillCategory: 'JAVA_SCRIPT',
        },
        {
            parentSkillCategory: 'PROGRAMMING_LANGUAGE',
            childSkillCategory: 'TYPE_SCRIPT',
        },
        {
            parentSkillCategory: 'WEB',
            childSkillCategory: 'NEXT_JS',
        },
        {
            parentSkillCategory: 'WEB',
            childSkillCategory: 'REACT',
        },
    ],
};
...
const getSingleQnAPostInfo: QueryFunction<QnAPostInfo, [_1: string, _2: string]> = ({ queryKey }) => {
    ...
    return axios.get(`/api/dev-post/${roomId}`).then(response => response.data);
    // console.log(postId);
    //
